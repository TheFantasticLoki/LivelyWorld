...
public class LivelyWorld : Script
{
    ...
    {
       new Vector3 (-144,-593,211),//tall
        new Vector3(-285,-618,50), //Daily Globe, Journalism
        new Vector3(478.4f, -3369.9f, 6f), //Port
        new Vector3(-475.5f, 5988.6f, 31.4f)//Paleto
    };
...
"G_F_Y_Lost_01" };
 ...
 new Vector3(-3880,-2390,1),
    ...
    void ProcessCheats()
    {
        if (CanWeUse(ttest)) UI.ShowSubtitle(ttest.IsAttached().ToString());
        if (WasCheatStringJustEntered("carattach"))
        {
            ttest = World.CreateVehicle("technical", Game.Player.Character.Position.Around(5), 0f);


            Attach(Game.Player.Character.CurrentVehicle, ttest);
            if (ttest.IsAttached()) ttest.Detach();

            
           AttachPhysically(ttest, Game.Player.Character.CurrentVehicle, Vector3.Zero, Vector3.Zero, Vector3.Zero, 0,0, 3000f,true, false, true, false);
            //p.AttachTo(Game.Player.Character.CurrentVehicle, Game.Player.Character.CurrentVehicle.GetBoneIndex("spoiler"));
            ttest.IsPersistent = false;
       ...
       */
        ...
        if (WasCheatStringJustEntered("lwpullover"))
        {
            Ped p = null;
        ...
        */
        ...
        if (WasCheatStringJustEntered("lwlogpos"))
        {
            Vector3 pos = Game.Player.Character.Position;
            string t = "";
            ...
            if (BarnCars.Count > 0)
            {
                ...
                foreach (Vehicle ve in BarnCars)
                {
                    if (CanWeUse(ve))
                    {
                        ...
                        {
                            BarnFindTutorial = Game.GameTime + 120000;
    ...
    {
     //   foreach (Vehicle v in ToAttachBikes) World.DrawMarker(MarkerType.ChevronUpx1, v.Position + (v.UpVector * 2), Vector3.Zero, -Vector3.WorldDown, new Vector3(1f, 1f, -1f), Color.SkyBlue);
        ...
        */
        ...
        if (GameTimeRefLong < Game.GameTime)
        {
            ...
            if (ToAttachBikes.Count > 20) ToAttachBikes.RemoveAt(0);
    ...
    List<Vehicle> ToAttachBikes = new List<Vehicle>();
    void AttackBikes()
    {
        if (ToAttachBikes.Count == 0) return;
        if (ScenarioFlow.Contains(ScenarioType.PoliceCarCarryingBike) && ScenarioFlow.Contains(ScenarioType.CarCarryingBike)) return;
        if (!CurrentlyAllowedScenarios.Contains(ScenarioType.PoliceCarCarryingBike) && !CurrentlyAllowedScenarios.Contains(ScenarioType.CarCarryingBike)) return;

        foreach (Vehicle veh in ToAttachBikes)
        {
            int Chance = 10;
            if (CanWeUse(veh) && !veh.IsAttached())
            {
                //Police carrying bikes

                if (new Model("cycle").IsValid && CurrentlyAllowedScenarios.Contains(ScenarioType.PoliceCarCarryingBike) && !ScenarioFlow.Contains(ScenarioType.PoliceCarCarryingBike) && isCopVehicleRange(veh.Position, 1f))
                {
        ...
        if (AllVehicles.Length > 0)
        {
            for (int i = 0; i < 20; i++)
            {
                ...
                if (CanWeUse(veh)  && !BlacklistedVehicles.Contains(veh) && !veh.PreviouslyOwnedByPlayer && !veh.IsPersistent && !DecorExistsOn("LWIgnore", veh)) // 
                {
                    ...
                    if(!veh.IsAttached() && IsSuitableForPlayerToExperience(veh.Position, InteractionRange) && veh.Model.IsCar && (isCopVehicleRange(veh.Position, 1f) || new List<VehicleClass> { VehicleClass.OffRoad, VehicleClass.Sedans, VehicleClass.SUVs }.Contains(veh.ClassType) == true))
                    {
                     if(!ToAttachBikes.Contains(veh))  ToAttachBikes.Add(veh);
                    }

                    //IsInNamedArea(Game.Player.Character, "Senora Fwy") || IsInNamedArea(Game.Player.Character, "Great Ocean Hwy")
                    if (CurrentlyAllowedScenarios.Contains(ScenarioType.ImprovedFreight) && !ScenarioFlow.Contains(ScenarioType.ImprovedFreight) && (IsInNamedArea(Game.Player.Character, "Senora Fwy") || IsInNamedArea(Game.Player.Character, "Great Ocean Hwy")) && veh.Speed < 25f && veh.HasBone("attach_female") && veh.ClassType == VehicleClass.Commercial &&
                    ...
                    {//(veh.Model == "juggernaut" || veh.Model == "packer" || veh.Model == "hauler" || veh.Model == "phantom" || veh.Model == "roadkiller") !DecorExistsOn("LWIgnore", veh) 
                        ...
                        if (!CanWeUse(trailer) && !Function.Call<bool>(Hash.IS_VEHICLE_ATTACHED_TO_TRAILER, veh))
                        {
                            ...
                            ScenarioFlow.Add(ScenarioType.ImprovedFreight);
                            continue;
                        }
                    }
    ...
    public bool VisibleToPlayer(Vector3 pos)
    {
        if (Game.Player.Character.IsInRangeOf(pos, 10f)) return true;


        //Player heading to pos
        if (Game.Player.Character.Velocity.Length() > 1f)
        {
    ...
    {
        if (Game.Player.Character.IsInRangeOf(pos, 10f)) return true;
        if (Function.Call<bool>(Hash.WOULD_ENTITY_BE_OCCLUDED, Game.GenerateHash("firetruk"), pos.X, pos.Y, pos.Z, true)) return false;
        else
        {
            if (pos.DistanceTo(Game.Player.Character.Position) > 150f) return false; else return true;
        }
    ...
    public static Vector3 GetQuietPlace()
    {
        ...
        for (int i = 0; i < 50; i++)
        {
            ...
            {
                if (pos.DistanceTo(Game.Player.Character.Position) > 50f)
                {
                    int covered = 0;
                    RaycastResult r = World.RaycastCapsule(pos + new Vector3(10, 0, 5), pos, 2f, IntersectOptions.Map);
                    if (r.DitHitAnything) covered += 25;
                    r = World.RaycastCapsule(pos + new Vector3(-10, 0, 5), pos, 2f, IntersectOptions.Map);
                    if (r.DitHitAnything) covered += 25;
                    r = World.RaycastCapsule(pos + new Vector3(0, 10, 5), pos, 2f, IntersectOptions.Map);
                    if (r.DitHitAnything) covered += 25;
                    r = World.RaycastCapsule(pos + new Vector3(0, -10, 5), pos, 2f, IntersectOptions.Map);
                    if (r.DitHitAnything) covered += 25;


                    if (covered >= 50) return pos;
                }
    ...
    public static void MoveEntitytoNearestRoad(Entity E)
    {
        if (CanWeUse(E))
        {
            ...
            if (Function.Call<bool>(Hash.GET_CLOSEST_VEHICLE_NODE_WITH_HEADING, E.Position.X, E.Position.Y, E.Position.Z, outArgA, outArgB, 0, 1077936128, 0))
            {
               ...
               // E.Position = outArgA.GetResult<Vector3>();
            }
    ...
    void LoadSettings()
    {
        ...
        Notify("", "", "", "Remember that you can edit ~g~"+ScriptName+ "'~w~s Settings in ~b~LivelyWorl.ini~w~ and ~b~LivelyWorld.xml~w~.");
    ...
    void AttachPhysically(Entity e, Entity carrier, Vector3 eOffset, Vector3 carrierOffset,  Vector3 eRotation, int eBone, int carrierBone, float force, bool Fixedrot, bool fixedpos ,bool collision, bool teleport)
    {
        Function.Call(Hash.ATTACH_ENTITY_TO_ENTITY_PHYSICALLY, e, carrier, 0, 0, carrierOffset.X, carrierOffset.Y, carrierOffset.Z, eOffset.X, eOffset.Y, eOffset.Z, eRotation.X, eRotation.Y, eRotation.Z, force, Fixedrot, fixedpos, collision, !teleport, 2); //+ (v.Model.GetDimensions().Y/2f)
